...
object Show {
  ...
  implicit val strShow: Show[String] = from[String](str => str)
  implicit val boolShow: Show[Boolean] = from[Boolean](_.toString)

  // 1.2 Instances with conditional implicit

  implicit def lstShow[A](implicit ev: Show[A]): Show[List[A]] =
    (a: List[A]) => a.mkString(";")
